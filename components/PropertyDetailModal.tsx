...
import { X, MapPin, Home, DollarSign, Phone, ExternalLink, Calendar, Building2 } from "lucide-react";
...
function Row({ label, value }: { label: string; value: string | number | undefined }) {
  ...
  return (
    <div className="flex justify-between items-baseline py-2.5" style={{ borderBottom: "1px solid #f0f0f0" }}>
      ...
      <span className="text-sm font-medium ml-4 text-right" style={{ color: "#111111" }}>{String(value)}</span>
    </div>
  );
}
...
export default function PropertyDetailModal({ property, onClose }: Props) {
  ...
  const mailCityState = [property.ownerMailingCity, property.ownerMailingState].filter(Boolean).join(", ");
  const mailZip = property.ownerMailingZip;

  // Build full mailing address for phone lookup
  const searchAddr = [mailLine1, mailCityState, mailZip].filter(Boolean).join(" ");
  const phoneLookupUrl = `https://www.fastpeoplesearch.com/address/${encodeURIComponent(
    [mailLine1 ?? property.address, mailCityState || (property.city + ", " + property.state), mailZip ?? property.zip].join(" ")
  )}`;
      ...
      >
        ...
        >
          <div className="min-w-0">
            <div className="flex items-center gap-2 font-semibold text-base leading-tight" style={{ color: "#111111" }}>
        ...
        <div className="flex-1 p-5 space-y-6">
          ...
          <div className="rounded-xl p-4 flex items-center justify-between" style={{ background: "#f5f5f5" }}>
            <div>
              <p className="text-xs font-medium uppercase tracking-wide mb-0.5" style={{ color: "#888888" }}>Estimated Value</p>
              <p className="text-2xl font-bold" style={{ color: "#111111" }}>{property.estimatedValue}</p>
            </div>
            <div className="text-right">
              <p className="text-xs font-medium uppercase tracking-wide mb-0.5" style={{ color: "#888888" }}>Est. Equity</p>
              <p className="text-2xl font-bold" style={{ color: "#15803d" }}>{property.equity}</p>
              <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: eqColors.bg, color: eqColors.color }}>
          ...
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: "#888888" }}>Owner Info</h3>
            <div className="rounded-xl p-4 space-y-3" style={{ border: "1px solid #e5e5e5" }}>
              <div>
                <p className="text-xs mb-0.5" style={{ color: "#888888" }}>Owner Type</p>
                <p className="text-sm font-medium" style={{ color: "#111111" }}>
                  {property.ownerType}
                  {property.isHomestead && (
                    <span className="ml-2 text-xs px-2 py-0.5 rounded-full" style={{ background: "#f0f0f0", color: "#555555" }}>
                      Owner-Occupied
                    </span>
                  )}
                </p>
              ...
              {(mailLine1 || mailCityState) && (
                <div>
                  ...
                  <div className="text-sm font-medium leading-relaxed" style={{ color: "#111111" }}>
                    ...
                    {(mailCityState || mailZip) && <p>{[mailCityState, mailZip].filter(Boolean).join(" ")}</p>}
                  </div>
                  {property.neighborhood && (
                    <p className="text-xs mt-1" style={{ color: "#888888" }}>Neighborhood: {property.neighborhood}</p>
                  )}
                </div>
              )}
              ...
              {/* Phone Lookup */}
              <div className="pt-1">
                <p className="text-xs mb-2" style={{ color: "#888888" }}>Phone Number</p>
                <p className="text-xs mb-2" style={{ color: "#aaaaaa" }}>
                  Not in public records — search by mailing address to skip trace.
                </p>
                <a
                  href={phoneLookupUrl}
                  target="_blank"
          ...
          {/* ── Property Details ── */}
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: "#888888" }}>Property Details</h3>
          ...
          {/* ── Sale History ── */}
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: "#888888" }}>Sale History</h3>
            <div className="rounded-xl px-4 py-1" style={{ border: "1px solid #e5e5e5" }}>
          ...
          {property.tags.length > 0 && (
            <div>
              <h3 className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: "#888888" }}>Flags</h3>
